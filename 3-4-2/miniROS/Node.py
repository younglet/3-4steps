...
class Node:
    ...
    def start(self):
        ...
        if not self.running:
            ...
            if self.on_setup:
                if self.main:
                    self.on_setup()  # 在主线程中运行
                else:
                    self.on_setup_thread = threading.Thread(target=self.on_setup)
                    self.on_setup_thread.start()
...
def set_task(node, *, loop=False, main=False, index=None):
    """
    装饰器：注册节点启动时的任务
    loop参数代表是否这个on_setup需要无限循环来进行处理
    main参数代表是否这个on_setup需要在主线程中运行
    """
    def decorator(func):
        def wrapper():
            if loop:
